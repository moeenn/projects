...
func Initialize(args *templates.TemplateArgs) error {
	fmt.Printf("Initializing new Typescript project: %s\n", args.ProjectName)
...
func initFiles(args *templates.TemplateArgs) error {
	...
	viteConfigJsPath := filepath.Join(args.RootPath, "vite.config.js")
	err = templates.CreateFileFromTemplate(args.Templates, viteConfigJsPath, "ts.vite_config_js", args)
	if err != nil {
		return err
	}
